...
/** 機械名は、まだ入力履歴が少ない導入初期でも使えるよう、既知の値をあらかじめ種として持たせておく。 */
var TOOL_FIELD_SEED_ = {
  // 実際に社内にある機械（機種＋機番）の一覧（2026-09-07、ユーザー提供）。
  '機械名': [
    'E20 No.8', 'RL No.25', 'RL No.26', 'RL No.27', 'RL No.28', 'M12 No.83',
    'A20 No.88', 'L16 No.29', 'L16 No.30', 'M16 No.87', 'B12 No.80', 'B12 No.81',
    'M32 No.86', 'M32 No.91', 'L25 No.2', 'M20 No.1', 'M12 No.82', 'M12 No.85',
    'L20 No.31', 'M32 No.93', 'M12 No.84', 'A32 No.92', 'C32 No.90', 'A20 No.89',
    'F25 No.3', 'F20 No.6', 'B12 No.21', 'B12 No.22', 'L20 No.32'
  ]
};
...
function getToolFieldSuggestions_() {
  ...
  if (lastRow >= 2) {
    var header = sheet.getRange(1, 1, 1, lastCol).getValues()[0];
    var cols = {};
    ['品番', '機械名'].forEach(function (name) { cols[name] = header.indexOf(name); });
...
/**
 * 検査記録フォルダの新規作成が「得意先フォルダも見つからない」と判定される実例の調査用
 * （2026-09-07、使い捨て）。GASエディタでこの関数を選んで実行し、実行数ログを確認すること。
 */
function diagnoseInspectionFolderCreate() {
  var zuban = '90007-FD3P-0000'; // 調査対象。別の図番を調べたい場合はここを書き換えて再実行する

  Logger.log('=== scanZubanMaster_（I-PROから品名・得意先コードを引く） ===');
  var master = scanZubanMaster_(zuban);
  Logger.log('hinmei=[' + master.hinmei + '] tokuisaki=[' + master.tokuisaki + ']');

  if (!master.tokuisaki) {
    Logger.log('得意先コードが空のため、ここで判定不能（I-PRO側にこの図番の得意先コードが無い）');
    return;
  }

  Logger.log('=== lookupTokuisakiName_（得意先コード→得意先名） ===');
  var tokuisakiName = lookupTokuisakiName_(master.tokuisaki);
  Logger.log('tokuisakiName=[' + tokuisakiName + ']');

  if (!tokuisakiName) {
    Logger.log('得意先名が空のため、ここで判定不能（進捗状況照会にこの得意先コードの行が無い）');
    return;
  }

  Logger.log('=== findExistingCompanyFolder_と同じクエリで完全一致検索 ===');
  var nameEsc = String(tokuisakiName).replace(/'/g, "\\'");
  var exact = driveFilesList_(
    "name = '" + nameEsc + "' and mimeType = 'application/vnd.google-apps.folder' and trashed = false"
  );
  Logger.log('件数: ' + exact.length);
  ...
  Logger.log('=== findExistingCompanyFolder_の実際の戻り値 ===');
  var result = findExistingCompanyFolder_(tokuisakiName);
  Logger.log(result ? ('見つかった: id=' + result.id + ' name=[' + result.name + ']') : '見つからなかった（null）');
}
...
/**
 * ocrToolLayout_のプロンプトで使う閉じた選択肢一覧。index.html側のCATEGORY_LIST／
 * SUBTYPES_BY_CATEGORY／mountのoptionsと完全一致させること（値を変えたら両方直す）。
 * 表記ゆれのままだと、アプリの選択式欄（工具種類・工具種類詳細・取付方式）に自動で
 * プリセットされず、読み取り結果が生かせなくなるため一致させる意味が大きい
 * （2026-09-14、入力項目が増えたのにOCRが対応していなかったため追加）。
 */
var OCR_TOOL_CATEGORY_LIST_ = [
  '突切', '前挽き', '後挽き', '外径溝', '内径溝', '端面溝', '外径ネジ', '内径ネジ', 'スレッドミル', '内径',
  'シェーパー', 'エンドミル', '面取カッター', 'スリ割カッター', 'センタードリル', 'スポットドリル', 'ドリル',
  'タップ', 'リーマー', '特殊工具', '転造ローレット', '切削ローレット', 'バリ取りツール'
];
var OCR_SUBTYPES_BY_CATEGORY_ = {
  '内径': ['内径ボーリング', '内径引き', '内径端面溝'],
  'エンドミル': ['スクエアエンドミル', 'ボールエンドミル', 'ラジアスエンドミル', 'Ｖカットエンドミル', 'ラフィングエンドミル', 'テーパーエンドミル'],
  'ドリル': ['ソリッドドリル', 'チップ交換式ドリル', 'ヘッド交換式ドリル'],
  '特殊工具': ['内径特殊', '外径特殊']
};
var OCR_MOUNT_LIST_ = ['シングル', 'ダブル', 'トリプル', '回転', 'クロス', '傾斜', 'クシ歯', 'B軸'];

/**
 * ④紙の取込確認：ツールレイアウト表（手書き含む）の写真をGemini APIに送り、
 * ③ツール配置画面の編集データと同じ形（正面/背面/サイクルチャック径・工具リスト・変更履歴メモ等）で
 * 読み取り結果を返す（2026-08-31、実データでのパイロットテストを経て導入。2026-09-14、工具種類詳細・
 * 主軸・取付方式・突切の方向/刃形・プログラム番号(サブ)等、後から増えた項目を読み取り対象に追加）。
...
function ocrToolLayout_(payload) {
  ...
  var prompt = multiNote +
    ...
    '  "programNumber": "プログラム番号（メイン）",\n' +
    '  "programNumberSub": "プログラム番号（サブ。複数ある場合は改行区切りで全て書く）",\n' +
    '  "positions": [ {\n' +
    '    "column": "front",\n' +
    '    "tNumber": "Tナンバー",\n' +
    '    "offsetNo": "オフセットNo.（工具補正番号。Tナンバーと同じ数字の場合も、書かれていればそのまま書く）",\n' +
    '    "category": "工具種類（下記一覧から紙面の表記に最も近いものを1つ選ぶ）",\n' +
    '    "subCategory": "工具種類の詳細（下記参照。対象4種類の工具種類の時だけ意味を持つ）",\n' +
    '    "detail": "詳細情報（型番・寸法等、工具種類に当てはまらない補足）",\n' +
    '    "shift": "シフト（工具のオフセット量・ズレ量）",\n' +
    '    "maker": "工具メーカー名",\n' +
    '    "partNumber": "工具の品番",\n' +
    '    "spindle": "主軸（正面/背面のどちらかが紙面に明記されている場合のみ）",\n' +
    '    "mount": "取付方式（下記一覧から紙面の表記に最も近いものを1つ選ぶ）",\n' +
    '    "mountPosition": "取付位置（上側/下側。取付方式がダブルの行でだけ意味を持つ）",\n' +
    '    "yAxis": "Y軸位置（Y-/Y0/Y+。取付方式がダブル・トリプルの行でだけ意味を持つ）",\n' +
    '    "coolantType": "クーラント種別（高圧/中圧のどちらかが紙面に明記されている場合のみ）",\n' +
    '    "mCode": "Mコード（クーラント等のMコード指定。数字のみで書かれていても先頭にMを付けて出力する）",\n' +
    '    "cutoffDirection": "方向（右/左。工具種類が突切の行でだけ意味を持つ）",\n' +
    '    "cutoffBladeType": "刃形（N/勝手付。工具種類が突切の行でだけ意味を持つ）"\n' +
    '  } ],\n' +
    ...
    'category（工具種類）は次の一覧から紙面の表記に最も近いものを1つ選んでください（一覧はアプリ側の選択候補と完全一致させる' +
    '必要があるため、表記ゆれがあっても最も近いものに寄せること）：' + OCR_TOOL_CATEGORY_LIST_.join('/') + '。' +
    '一覧のどれにも当てはまらない場合のみ、紙面の表記をそのまま書いてください。\n' +
    'subCategory（工具種類の詳細）は、categoryの値に応じて次から選んでください（それ以外のcategoryでは空文字）：' +
    Object.keys(OCR_SUBTYPES_BY_CATEGORY_).map(function (c) {
      return 'category=「' + c + '」の場合は[' + OCR_SUBTYPES_BY_CATEGORY_[c].join('/') + ']';
    }).join('、') + '。\n' +
    'mount（取付方式）は次の一覧から選んでください：' + OCR_MOUNT_LIST_.join('/') + '。\n' +
    'category/subCategory/detail/shift/maker/partNumber/spindle/mount/mountPosition/yAxis/coolantType/mCode/' +
    'cutoffDirection/cutoffBladeType/offsetNo/programNumber/programNumberSubは、紙面にその情報が明記されている' +
    '場合のみ埋め、書かれていない・読み取れない・その行の工具種類に当てはまらない項目は空文字にしてください' +
    '（無理に推測しない）。';
